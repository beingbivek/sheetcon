...
type StatusFilter = 'ALL' | 'AWAITING_PAYMENT' | 'UNDER_REVIEW' | 'APPROVED' | 'REJECTED' | 'EXPIRED';
...
export default function PaymentsClient({ payments: initialPayments }: PaymentsClientProps) {
  ...
  const handleAction = async (action: 'approve' | 'reject') => {
    ...
    try {
      ...
      // Update local state so table refreshes without full page reload
              ...
              {filtered.map(payment => (
                <tr key={payment.id} className="hover:bg-slate-700/50 transition-colors">
                  ...
                  <td className="px-4 py-3 text-slate-300">
                    ...
                    <span className="text-slate-500 text-xs ml-1">({payment.requestedBillingCycle})</span>
                  </td>
                  <td className="px-4 py-3 text-slate-200 font-medium">
                    {payment.currency} {payment.amountSnapshot}
                  </td>
                  <td className="px-4 py-3">
                    <span className={`px-2 py-1 rounded-full text-xs font-medium ${STATUS_COLORS[payment.status] || STATUS_COLORS.EXPIRED}`}>
          ...
          <div className="bg-slate-800 rounded-xl border border-slate-700 w-full max-w-2xl max-h-[90vh] overflow-y-auto">
            <div className="p-6">
              ...
              <div className="flex items-start justify-between mb-6">
                ...
                <button onClick={closeDetail} className="text-slate-400 hover:text-white text-2xl leading-none">×</button>
              </div>
              ...
              <div className="grid grid-cols-2 gap-4 bg-slate-900 rounded-lg p-4 mb-6 text-sm">
                ...
                <div>
                  ...
                  <p className="text-white font-bold text-lg">{selectedPayment.currency} {selectedPayment.amountSnapshot}</p>
                </div>
                <div>
                  <p className="text-slate-500">Status</p>
                  <span className={`px-2 py-1 rounded-full text-xs font-medium ${STATUS_COLORS[selectedPayment.status]}`}>
                ...
                {selectedPayment.transactionReference && (
                  <div>
                    ...
                    <p className="text-white font-mono text-sm">{selectedPayment.transactionReference}</p>
                  </div>
                )}
                ...
                <div>
                  ...
                  <p className={`${new Date(selectedPayment.expiresAt) < new Date() ? 'text-red-400' : 'text-white'}`}>
              ...
              {/* Already resolved */}
              {['APPROVED', 'REJECTED', 'EXPIRED', 'CANCELLED'].includes(selectedPayment.status) ? (
                <div className={`p-4 rounded-lg border text-sm ${
                  ...
                  {selectedPayment.status === 'APPROVED' && (
                    <p>✅ Approved{selectedPayment.approvedBy ? ` by ${selectedPayment.approvedBy.name}` : ''}</p>
                  )}
                  {selectedPayment.status === 'REJECTED' && (
                    <>
                      <p>❌ Rejected{selectedPayment.rejectedBy ? ` by ${selectedPayment.rejectedBy.name}` : ''}</p>
                      {selectedPayment.rejectionReason && (
                        <p className="mt-1 text-red-400">Reason: {selectedPayment.rejectionReason}</p>
                      )}
                    </>
                  )}
                  {selectedPayment.adminNote && (
                    <p className="mt-2 text-slate-400">Admin note: {selectedPayment.adminNote}</p>
                  )}
                </div>
              ) : (
                /* Action Panel */
                <div className="space-y-4">
                  <div>
                    <label className="block text-sm text-slate-400 mb-1">Admin Note (optional)</label>
                    <textarea
                      value={adminNote}
